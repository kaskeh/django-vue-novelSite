...
class emailCodeAPIView(APIView):
    def get(self, request, email):
        ...
        if ret is not None:
            return Response({"message": "验证码60秒内已经发生过，请耐心等待"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            # 内部错误
            return Response({"message": "发送验证码失败"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        # 5. 响应发送验证码的结构
        return Response({"message": "发送验证码成功！", "code_life": constants.SMS_INTERVAL_TIME}, status=status.HTTP_200_OK)
